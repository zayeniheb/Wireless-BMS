...
voltage2 = fabs(ads1115_get_voltage(&ads));
  //printf("voltage1:%f\n",voltage1);
    // printf("voltage2:%f\n",voltage2);
	 ...
	 else if ((voltage2-voltage1>0)&&(voltage2-voltage1<=0.2)){
		voltage1=0;
		 n=1;
	 }
	  else if ((voltage1-voltage2>0)&&(voltage1-voltage2<=0.2)){
		voltage2=0;
		 n=1;
	 }
	 else {
     ...
     ////////////////////////////////WARNINGS!!!!!!!!!!!!!//////////////////////////////////////
       if (voltage1<1){
		  strcpy(Tx_msg1," not connected" ) ;
		   ...
		   else if (voltage1<=2.17&&voltage1>1){
		     strcpy(Tx_msg2," under voltage" ) ;}
	   ...
	   if (voltage2<1){
	    ...
	    else if (voltage2<=2.17&&voltage2>1){
		     strcpy(Tx_msg2," under voltage" ) ;
	   ...
	   else{strcpy(Tx_msg2," conected and stable" ) ;
	   
	   }
	   if (i>=5&&voltage1<4&&voltage1!=0&&voltage2<4&&voltage2!=0){
		   for(int k=1;k<5;k++){
			   for(int j=k+1;j<5;j++){
				  
        float result1= fabs(CEEL1_TABLE[k]-CEEL1_TABLE[j]);
        float result2=fabs( CEEL2_TABLE[k]-CEEL2_TABLE[j]);
        if(result1>=1.0f){
			 strcpy(Tx_msg11," damaged" ) ;
		 
		}else{strcpy(Tx_msg11,"not damaged" ) ;}
		 if(result2>=1.0f){
			 strcpy(Tx_msg22," damaged" ) ;
		
		}else {strcpy(Tx_msg22," not damaged" ) ;}
