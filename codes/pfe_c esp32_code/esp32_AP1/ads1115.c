...
#include "esp_log.h"
#define GPIO_INTR_PIN GPIO_NUM_4
static const char *TAG = "GPIO_ISR_ADC";
static void IRAM_ATTR gpio_isr_handler(void* arg) {
  ...
  int pin = (int) arg;
    ESP_LOGI(TAG, "Interrupt triggered on GPIO pin %d", pin);
}
...
ads1115_t ads1115_config(i2c_port_t i2c_port, uint8_t address) {
  ...
  ads.rdy_pin.in_use = 1; // state that rdy_pin not used
  ads.last_reg = ADS1115_MAX_REGISTER_ADDR; // say that we accessed invalid register last
  ads.changed = 0; // say we changed the configuration
...
int16_t ads1115_get_raw(ads1115_t* ads) {
  ...
  if(ads->rdy_pin.in_use) {
    gpio_isr_handler_add(ads->rdy_pin.in_use , gpio_isr_handler, (void*)ads->rdy_pin.pin);
   /*/ xQueueReset(ads->rdy_pin.gpio_evt_queue)*/;
    ESP_LOGI(TAG, "Configuring ISR for GPIO: %d", ads->rdy_pin.pin);
  }
